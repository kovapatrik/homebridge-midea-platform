...
export abstract class MessageSubProtocol extends MessageACBase {
  ...
  get _body() {
    let body = Buffer.from([
      ...
      0x00, 0xFF, 0xFF, this.subprotocol_query_type,
    ]);
...
export class MessageNewProtocolSet extends MessageACBase {
  ...
  get _body() {
    ...
    if (this.fresh_air_1 !== undefined && this.fresh_air_1.length === 2) {
      ...
      payload = Buffer.concat([payload,
        NewProtocolMessageBody.packet(NewProtocolTags.FRESH_AIR_1, Buffer.from([
          fresh_air_power,
          fresh_air_fan_speed,
          0x00, 0x00, 0x00, 0x00,
          0x00, 0x00, 0x00, 0x00,
